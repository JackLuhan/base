package cn.houyi.base.config.interceptor;

import cn.houyi.base.common.annotation.RateLimiter;
import cn.houyi.base.common.constant.CommonConstant;
import cn.houyi.base.common.limit.RedisRaterLimiter;
import cn.houyi.base.common.utils.IpInfoUtil;
...
public class LimitRaterInterceptor extends HandlerInterceptorAdapter {
    ...
    @Value("${base.rateLimit.enable}")
    private boolean rateLimitEnable;

    @Value("${base.rateLimit.limit}")
    private Integer limit;

    @Value("${base.rateLimit.timeout}")
        ...
        try {
            ...
            if (rateLimiter != null) {
                ...
                if (StrUtil.isBlank(token3)) {
                    throw new GloabException("当前访问人数太多啦，请稍后再试");
                }
            }
        }catch (Exception e){
